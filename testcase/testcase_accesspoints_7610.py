...
class TestAccessPoints(unittest.TestCase):
    ...
    def test_026_check_2g4_n(self):
        ...
        result1 = tmp.connect_WPA_AP(data_wireless['all_ssid'],\
                        data_wireless["short_wpa"],data_basic["wlan_pc"])
        assert ("195" in result) and (data_wireless['all_ssid'] in result1),"test set AP 802.11n,fail!"
    ...
    def test_028_check_5g_ac(self):
        ...
        tmp2 = SSIDBusiness(self.driver)
        tmp2.change_AP_Freq("2.4GHz")
        assert ("1.3" in result) and (data_wireless['all_ssid'] in result1),"test set AP 802.11ac,fail!"
    ...
    #2.4G，stream选择3,自定义功率分别配置1-31,后台查看是否都对应生效
    def test_082_check_2g4_stream3_custom_power(self):
        u"""2.4G，stream选择3,自定义功率分别配置1-31,后台查看是否都对应生效(testlink_ID:2231)--bug84723"""
        tmp = APSBusiness(self.driver)
        #选择2.4g的激活空间流3
        tmp.change_2g4_active_streams("3")
        #自定义功率分别配置功率
        ...
        result = tmp.check_ap_2g4_custom_power(data_AP["master:mac"],
            powers,data_basic['DUT_ip'],data_basic['sshUser'], data_login['all'])
        self.assertNotIn(False, result), "set 2.4G stream3,choose custom power from 1 to 31,test fail!"
        print "set 2.4G stream3,choose custom power from 1 to 31,test pass!"
    ...
    #5G，stream选择3,自定义功率分别配置1-31,后台查看是否都对应生效
    def test_085_check_5g_stream3_custom_power(self):
        u"""5G，stream选择3,自定义功率分别配置1-31,后台查看是否都对应生效(testlink_ID:2234)--bug84723"""
        tmp = APSBusiness(self.driver)
        #选择5g的激活空间流3
        tmp.change_5g_active_streams("3")
        #自定义功率分别配置功率
        powers = data_AP['stream_powers']
        #验证ap的5G的自定义功率值
        result = tmp.check_ap_5g_custom_power(data_AP["master:mac"],
            powers,data_basic['DUT_ip'],data_basic['sshUser'], data_login['all'])
        self.assertNotIn(False, result), "set 5G stream3,choose custom power from 1 to 31,test fail!"
        print "set 5G stream3,choose custom power from 1 to 31,test pass!"
